...
class MT5FineTuner(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        ...
        self.log("loss", loss, prog_bar=self.hparams.progress_bar)
        if not self.hparams.progress_bar:
            print(f'Epoch {self.current_epoch} train_loss {loss} train_PPL {math.exp(loss)}')
        output_dir = f'{self.hparams.output_dir}_{self.current_epoch}'
